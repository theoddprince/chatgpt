...
RwBool CQuestIndicatorGui::Create( CQuestGui* pQuestGui )
{
	...
	//Amir - this class for the Quests list on the side of the screen
	...
	//Amir - I use this for debug to check the components 
	m_getAllComponents = (gui::CComponentManager*)GetAllComponents();
	...
	//Amir - this variable will only make the clipping area for how many items can be accessed at once
	...
	for(int i = 0 ; i < eMAX_CAN_PROGRESS_QUEST_NUM; i++)
	{
		...
		//Amir - Bug fix for quests out of boundary
		CheckQuestItemOutOfBoundary(m_IndicatorItem[i].m_pFrame->GetPosition(), m_ppnIndicatorList->GetPosition(), i);
...
VOID CQuestIndicatorGui::CheckQuestItemOutOfBoundary(CRectangle rect, CRectangle rect2, int i)
{
	CRectangle listRect = GetPosition();

	/*char acBuffer[256] = "";
	sprintf_s(acBuffer, "Item in index : %d position is : %d ", i, listRect.top + rect.top + 25 + rect2.top);
	NtlLogFilePrint(acBuffer);
	sprintf_s(acBuffer, "IndicatorList top is : %d ", rect2.top);
	NtlLogFilePrint(acBuffer);
	sprintf_s(acBuffer, "IndicatorList bottom is : %d ", rect2.bottom);
	NtlLogFilePrint(acBuffer);
	sprintf_s(acBuffer, "rect.top is : %d ", rect.top);
	NtlLogFilePrint(acBuffer);
	sprintf_s(acBuffer, "m_IndicatorItem[i].m_Background.GetPositionY() : %d ", m_IndicatorItem[i].m_Background.GetPositionY());
	NtlLogFilePrint(acBuffer);*/
	

	if (203 + rect.top + 25 + rect2.top < 203) //this means the item is out of list rectangle from the top
	{
		m_IndicatorItem[i].m_Background.Show(false);
		m_IndicatorItem[i].m_pTitle->Show(false);
		m_IndicatorItem[i].m_pbtnExpend->Show(false);
		m_IndicatorItem[i].m_pState->Show(false);
		m_IndicatorItem[i].m_pTime->Show(false);
		m_IndicatorItem[i].m_Background.SetAlpha(0);
	...
	else if (203 + rect.top + 50 + rect2.top > 573)//this means the item is out of list rectangle from the bottom
	{
		m_IndicatorItem[i].m_Background.Show(false);
		m_IndicatorItem[i].m_pTitle->Show(false);
		m_IndicatorItem[i].m_pbtnExpend->Show(false);
	...
	{
		m_IndicatorItem[i].m_Background.Show(true);
		m_IndicatorItem[i].m_pbtnExpend->Show(true);
		m_IndicatorItem[i].m_pState->Show(true);
		m_IndicatorItem[i].m_pTime->Show(true);
		m_IndicatorItem[i].m_Background.SetAlpha(50);
		...
		for (int j = 0; j < QUEST_MAX_GOAL_COUNT; j++)
		{
			m_IndicatorItem[i].m_pGoalInfo[j]->Show(true);
			m_IndicatorItem[i].m_pGoalResult[j]->Show(true);
			m_IndicatorItem[i].m_pGoalInfo[j]->Enable(true);
			m_IndicatorItem[i].m_pGoalResult[j]->Enable(true);
		}
		
		m_IndicatorItem[i].m_pTitle->Show(true);
		m_IndicatorItem[i].m_bQuestInfoShown = true;
		
		m_IndicatorItem[i].m_pTitle->Enable(true);
	}
}
...
;
		...
		m_IndicatorItem[i].m_Background.SetPosition(listRect.left + rect.left, 203 + rect.top + 25 + rect2.top);
		nYPos += rect.GetHeight() + QUEST_INDICATOR_ITEM_MARGIN;	

		//The log is located inside the folder of DragonBall under the name dbolog.txt
		/*char acBuffer[256] = "";
		sprintf_s(acBuffer, "Item in index : %d position is : %d ", i, listRect.top + rect.top + 25 + rect2.top);
		NtlLogFilePrint(acBuffer);
		sprintf_s(acBuffer, "IndicatorList top is : %d ", rect2.top);
		...
		sprintf_s(acBuffer, "IndicatorList bottom is : %d ", rect2.bottom);
		NtlLogFilePrint(acBuffer);
		sprintf_s(acBuffer, "listRect.top is : %d ", listRect.top);
		NtlLogFilePrint(acBuffer);
		sprintf_s(acBuffer, "listRect.bottom is : %d ", listRect.bottom);
		NtlLogFilePrint(acBuffer);*/

		//Amir - bug fix
		CheckQuestItemOutOfBoundary(rect, rect2, i);
